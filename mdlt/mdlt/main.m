...
% Images to stitch.
%------------------
% path1 = 'images/images/images/DSC02932.JPG';
% path2 = 'images/images/images/DSC02933.JPG';
path1='C:/Users/zyf/Desktop/photo/1/first.jpg';
path2='C:/Users/zyf/Desktop/photo/1/second.jpg';
%-------------
% Read images.
%-------------
...
% data_norm = [ dat_norm_img2 ; dat_norm_img1 ];
...
% cw = max([1 size(img2,2) TL(1) BL(1) TR(1) BR(1)]) - min([1 size(img2,2) TL(1) BL(1) TR(1) BR(1)]) + 1;
% ch = max([1 size(img2,1) TL(2) BL(2) TR(2) BR(2)]) - min([1 size(img2,1) TL(2) BL(2) TR(2) BR(2)]) + 1;
fprintf('done (%fs)\n',toc);

% Offset for left image.
fprintf('> Getting offset...');tic;
disp(min([1 size(img2,2) TL(1) BL(1) TR(1) BR(1)]));
disp(min([1 size(img2,1) TL(2) BL(2) TR(2) BR(2)]));
off = [ 1 - min([1 size(img1,2) TL(1) BL(1) TR(1) BR(1)]) + 1 ; 1 - min([1 size(img1,1) TL(2) BL(2) TR(2) BR(2)]) + 1 ];
% off = [ 1 - min([1 size(img2,2) TL(1) BL(1) TR(1) BR(1)]) + 1 ; 1 - min([1 size(img2,1) TL(2) BL(2) TR(2) BR(2)]) + 1 ];
fprintf('done (%fs)\n',toc);
disp([off(1),off(2)]);
%--------------------------------------------
...
% warped_img1(off(2):(off(2)+size(img2,1)-1),off(1):off(1)+size(img2,2)-1,:) = img2;
warped_img2 = imagewarping(double(ch),double(cw),double(img2),Hg,double(off));
% warped_img2 = imagewarping(double(ch),double(cw),double(img1),Hg,double(off));
warped_img2 = reshape(uint8(warped_img2),size(warped_img2,1),size(warped_img2,2)/3,3);
disp([size(warped_img1) size(warped_img2)]);
fprintf('done (%fs)\n',toc);
...
% Kp = [data_orig(5,inliers)' data_orig(6,inliers)'];
...
% [warped_img2] = imagewarping(double(ch),double(cw),double(img1),Hmdlt,double(off),X(1,:),Y(:,1)');
warped_img2 = reshape(uint8(warped_img2),size(warped_img2,1),size(warped_img2,2)/3,3);
fprintf('done (%fs)\n',toc);

figure;
imshow(warped_img2);
imwrite(warped_img1,'first_warp.jpg');
imwrite(warped_img2,'second_warp.jpg');
title('warpedimg2');

% leftoverlap=uint8(zeros(size(img1,1),size(img1,2)-min([TL(1) BL(1)])+1,3));
leftoverlap=img1(1:size(img1,1),min([TL(1) BL(1)]):size(img1,2),1:3);
rightoverlap=warped_img2(off(2):size(img1,1)+off(2)-1,min([TL(1) BL(1)]):size(img1,2),1:3);
% rightoverlap=img2(1:size(img2,1),1:min([TR(1) BR(1)]),1:3);
% leftoverlap=warped_img2(off(2):size(img1,1)+off(2)-1,off(1):min([TR(1) BR(1)])+off(1)-1,1:3);
figure;
imshow([leftoverlap,rightoverlap]);
title('overlap');
% overlap=SeamCarving(leftoverlap,rightoverlap);
% warped_img2(off(2):(off(2)+size(img1,1)-1),1:min([TL(1) BL(1)]),1:3)=img1(1:size(img1,1),1:min([TL(1) BL(1)]),1:3);
% warped_img2(off(2):size(img1,1)+off(2)-1,min([TL(1) BL(1)]):size(img1,2),1:3)=overlap;
% % warped_img2(off(2):(off(2)+size(img1,1)-1),off(1)+min([TR(1) BR(1)])-1:cw,1:3)=img2(1:size(img2,1),min([TR(1) BR(1)]):size(img2,2),1:3);
% % warped_img2(off(2):size(img1,1)+off(2)-1,off(1):min([TR(1) BR(1)])+off(1)-1,1:3)=overlap;
% figure;
% imshow(warped_img2);
% title('seam-carving')
% imwrite(warped_img2,'apap_seam.jpg');
